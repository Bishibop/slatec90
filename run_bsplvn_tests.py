...
for idx, test in enumerate(test_cases):
    ...
    fortran_code += f"    do j = 1, {test['vnikx_size']}\n"
...
with open('test_bsplvn_all.f90', 'w') as f:
    f.write(fortran_code)

# Compile and run
print("Compiling test program...")
compile_result = subprocess.run(['gfortran', '-o', 'test_bsplvn_all', 
                                'modern/bsplvn_modern.f90', 'test_bsplvn_all.f90'],
...
print("Running tests...")
run_result = subprocess.run(['./test_bsplvn_all'], capture_output=True, text=True)
...
print(f"Generated results for {len(test_cases)} test cases")
print("Results saved to bsplvn_results.txt")

# Clean up
os.remove('test_bsplvn_all.f90')
os.remove('test_bsplvn_all')