#!/usr/bin/env python3
"""Test modern CDIV implementation against validated test cases"""

import json
import subprocess
import re
from pathlib import Path

def generate_f90_cdiv_test(test_cases, batch_start, batch_size):
    """Generate F90 program for testing modern CDIV"""
    
    program = """program test_cdiv_batch
    use cdiv_module, only: cdiv
    implicit none
    
    real :: ar, ai, br, bi, cr, ci
    
"""
    batch_end = min(batch_start + batch_size, len(test_cases))
    
    for i in range(batch_start, batch_end):
        test = test_cases[i]
        ar, ai, br, bi = test['inputs']
        # Use index i+1 for 1-based numbering
        program += f"""    ! Test {i+1}
    ar = {ar:e}
    ai = {ai:e}
    br = {br:e}
    bi = {bi:e}
    call cdiv(ar, ai, br, bi, cr, ci)
    write(*,'(A,I5,A,E20.10,A,E20.10)') 'TEST_', {i+1}, &
        '_RESULT: ', cr, ', ', ci
    
"""
    program += "end program test_cdiv_batch"
    return program, batch_end - batch_start

def run_batch_tests(test_cases, batch_size=50):
    """Run tests in batches to avoid program size limits"""
    
    all_results = []
    total_tests = len(test_cases)
    
    for batch_start in range(0, total_tests, batch_size):
        print(f"Running batch starting at test {batch_start+1}...")
        
        # Generate program for this batch
        program, batch_count = generate_f90_cdiv_test(test_cases, batch_start, batch_size)
        
        # Write test program
        test_file = "test_cdiv_batch.f90"
        with open(test_file, 'w') as f:
            f.write(program)
        
        # Compile with modern module
        module_file = "modern/cdiv_modern.f90"
        exe_file = "test_cdiv_exe"
        
        compile_cmd = ['gfortran', '-o', exe_file, module_file, test_file]
        result = subprocess.run(compile_cmd, capture_output=True, text=True)
        
        if result.returncode != 0:
            print(f"Compilation failed: {result.stderr}")
            return None
        
        # Run
        run_result = subprocess.run([f'./{exe_file}'], capture_output=True, text=True)
        
        if run_result.returncode != 0:
            print(f"Execution failed: {run_result.stderr}")
            return None
        
        # Parse output - handle two values (real and imaginary parts)
        output = run_result.stdout
        batch_results = []
        for line in output.split('\n'):
            match = re.search(r'TEST_\s*(\d+)_RESULT:\s*([-+]?\d*\.?\d+[eE][-+]?\d+),\s*([-+]?\d*\.?\d+[eE][-+]?\d+)', line)
            if match:
                test_num = int(match.group(1))
                cr = float(match.group(2))
                ci = float(match.group(3))
                batch_results.append((test_num, cr, ci))
        
        all_results.extend(batch_results)
        
        # Cleanup
        Path(test_file).unlink(missing_ok=True)
        Path(exe_file).unlink(missing_ok=True)
        Path("cdiv_module.mod").unlink(missing_ok=True)
        
        print(f"  Got {len(batch_results)} results")
    
    return all_results

# Load test cases
with open('test_data/cdiv_tests.json', 'r') as f:
    test_data = json.load(f)
    test_cases = test_data['test_cases']

print(f"Loaded {len(test_cases)} test cases")
print("Testing modern CDIV implementation...")

# Run all tests
results = run_batch_tests(test_cases)

if results:
    print(f"\nGot {len(results)} results total")
    
    # Verify results match expected
    tolerance = 1e-6  # Relative tolerance for float comparison
    failures = 0
    
    for (test_num, cr_actual, ci_actual), test_case in zip(results, test_cases):
        cr_expected, ci_expected = test_case['expected']
        
        # Check relative error for real part
        if cr_expected != 0:
            rel_error_r = abs(cr_actual - cr_expected) / abs(cr_expected)
        else:
            rel_error_r = abs(cr_actual - cr_expected)
        
        # Check relative error for imaginary part  
        if ci_expected != 0:
            rel_error_i = abs(ci_actual - ci_expected) / abs(ci_expected)
        else:
            rel_error_i = abs(ci_actual - ci_expected)
        
        if rel_error_r > tolerance or rel_error_i > tolerance:
            failures += 1
            if failures <= 10:  # Show first 10 failures
                print(f"\nTest {test_num} FAILED:")
                print(f"  Description: {test_case['description']}")
                print(f"  Inputs: {test_case['inputs']}")
                print(f"  Expected: ({cr_expected}, {ci_expected})")
                print(f"  Actual: ({cr_actual}, {ci_actual})")
                print(f"  Relative errors: real={rel_error_r}, imag={rel_error_i}")
    
    print(f"\n{len(results) - failures} tests PASSED")
    print(f"{failures} tests FAILED")
    
    if failures == 0:
        print("\n✓ Modern CDIV implementation is correct!")
    else:
        print("\n✗ Modern CDIV implementation has errors!")