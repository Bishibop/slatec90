...
25      continue
        ...
        print *, "j=", j, "ipj=", ipj, "imjp1=", imjp1
        print *, "  t(ipj)=", t(ipj), "t(imjp1)=", t(imjp1)
        print *, "  deltap(j)=", deltap(j), "deltam(j)=", deltam(j)
        ...
        do l = 1, j
            ...
            print *, "  l=", l, "deltap(l)=", deltap(l), "deltam(jp1ml)=", deltam(jp1ml), "denom=", denom
            
            if (abs(denom) < 1e-10) then
                print *, "  WARNING: Near-zero denominator!"
                vm = 0.0  ! Handle division by zero
            else
                vm = vnikx(l) / denom
            end if
